...
//  Web3Boiler
//
//  Created by Pavel Pushkarev on 30/1/23.
//
...
public final class TorusWeb3Utils {
    ...
    private static let etherInMwei = pow(Double(10), 6)
    ...
    public static func fromStable(amount: BigUInt) -> Ether {
        guard let decimal = Double(amount.description) else {
            return 0
        }
        return decimal / etherInMwei
    }
    
    /// Convert Stable(Decimal) unit to (BInt) unit
    public static func toStable(amount: Double) -> Wei {
        let wei = Wei(amount * etherInMwei)
        return wei
    }
